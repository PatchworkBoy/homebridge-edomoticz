      ...
      if (this.isSwitch)
      {
		...
		case this.Type == "Wind":{
        		var windServiceCurrentCharacteristic = this.getService(eDomoticzServices.WindDeviceService).getCharacteristic(Characteristic.CurrentTemperature);
                	...
                	var windeg = (string)message.svalue1;
                	var windir = (string)message.svalue2;
                	var newval2 = windeg+' '+windir;
                	callback(windServiceWindDirectionCharacteristic,newval2);
		...
		case this.Type == "Temp" || this.Type == "Temp + Humidity" || this.Type == "Temp + Humidity + Baro":{
	        ...
	        case this.Type == "P1 Smart Meter":{
                if (this.subType == "Gas"){
                  ...
                  var newval = (message.svalue1 / 1000)
                  callback(P1GasMeterDeviceCharacteristic,newval);
