...
function eDomoticzAccessory(platform, platformAccessory, IsScene, status, idx, name, uuid, haveDimmer, maxDimLevel, subType, Type, batteryRef, swType, swTypeVal, hwType, eve) {
...
eDomoticzAccessory.prototype = {
	...
	getService: function(name) {
		var service = this.platformAccessory.getService(name);
		if (!service)
		{
	...
	setBlindStatus: function(blindService, pos, callback, context) {
		...
		var finish = function(position) {
			...
			if (!this.isPercentageBlind) {
				position = (shouldOpen ? 100 : 0);
			}

			blindService.getCharacteristic(Characteristic.CurrentPosition).setValue(position, false, this);
		}.bind(this);
	...
	setLockStatus: function(doorstate, callback, context) {
		...
		Domoticz.updateDeviceStatus(this, "switchlight", {"switchcmd": (command ? "On" : "Off")}, function(success) {
			callback();
		}.bind(this));
	},
      ...
      case this.swTypeVal == Constants.DeviceTypeSwitch || this.swTypeVal == Constants.DeviceTypePushOn:
        {
          ...
          break;
			...
			case this.swTypeVal == Constants.DeviceTypeContact:
				{
					var characteristic = this.getService(Service.ContactSensor).getCharacteristic(Characteristic.ContactSensorState);
					callback(characteristic, message.nvalue);
					break;
				}
			...
			case this.swTypeVal == Constants.DeviceTypeDoorContact:
				{
					...
					var targetCharacteristic = service.getCharacteristic(Characteristic.LockTargetState);
					callback(targetCharacteristic, doorState);

					var currentCharacteristic = service.getCharacteristic(Characteristic.LockCurrentState);
			...
			case this.Type == "Humidity":
				{
					...
					{
						var characteristic = this.getService(Service.HumiditySensor).getCharacteristic(Characteristic.CurrentRelativeHumidity);
	...
	getServices: function() {
		...
		} else // Accessory is a sensor
		{
			...
			case this.Type == "Humidity":
			{
				...
				service.getCharacteristic(Characteristic.CurrentRelativeHumidity).on('get', this.getHumidity.bind(this));
				this.services.push(service);
				break;
			}
