...
function eDomoticzAccessory(platform, baseURL, IsScene, status, idx, name, haveDimmer, maxDimLevel, subType, Type, batteryRef, swType, swTypeVal, hwType, eve) {
...
eDomoticzAccessory.prototype = {
	...
	setdValue: function(level, callback, context) {
		...
		var dim = Math.floor((level / this.factor == 15) ? 16 : level / this.factor);
	...
	setPoint: function(setpoint, callback, context) {
		...
		if (this.subType == "SetPoint") {
			url = this.baseURL + "type=command&param=udevice&idx=" + this.idx;
			url = url + "&nvalue=0&svalue=" + setpoint;
		} else if (this.subType == "Zone") {
			url = this.baseURL + "type=setused&idx=" + this.idx + "&setpoint=";
