...
function eDomoticzAccessory(platform, server, port, IsScene, status, idx, name, haveDimmer, maxDimLevel, subType, Type, batteryRef, authorizationToken, swType, swTypeVal, prot, hwType, eve) {
	...
	Domoticz.initialize(this, prot, this.requestHeaders);
...
eDomoticzAccessory.prototype = {
	...
	setPowerState: function(powerOn, callback, context) {
		this.cachedValues[Characteristic.On] = powerOn;
	...
	getPowerState: function(callback) {
		var cachedValue = this.cachedValues[Characteristic.On];
		if (cachedValue) {
		...
		Domoticz.deviceStatus(this, function(json) {
			...
			this.cachedValues[Characteristic.On] = value;
		}.bind(this));
	},
	...
	getdValue: function(callback) {
		Domoticz.deviceStatus(this, function(json) {
			...
			sArray.map(function(s) {
				this.factor = 100 / s.MaxDimLevel;
				value = (s.Status == "Off" ? 0 : s.LevelInt * this.factor);
			}.bind(this));
			this.platform.log("Data Received for " + this.name + ": " + value);
			this.isOn = (value > 0);
			callback(null, value);
	...
	setHueValue: function(type, value, callback, context) {
		...
		if (this.hueValue !== undefined && this.saturationValue !== undefined && this.hueSemaphore !== undefined && this.hueSemaphore > 0)
		{
			...
			Domoticz.updateDeviceStatus(this, "switchlight", {"switchcmd": "setcolbrightnessvalue", "hex": Helper.HSVtoRGB([this.hueValue, this.saturationValue, 100])}, function(success) {
				callback();
			}.bind(this));
		} else {
			callback();
			...
			case this.swTypeVal == Constants.DeviceTypeMedia:
				{
					...
					if (message.svalue1 == 0 || this.isOn != isOn) {
						callback(powerCharacteristic, isOn);
						this.isOn = isOn;
					}

					if (this.isOn && this.factor)
					{
						var brightness = Math.round(message.svalue1 * this.factor);
						if (brightness > 0) {
			...
			case this.Type == "General" || this.Type == "YouLess Meter" || this.Type == "Current" || this.Type == "UV" || this.Type == "Usage" || this.Type == "Lux":
				{
					if ('undefined' !== typeof message.svalue1) {
						if (this.subType == "kWh" || this.subType == "YouLess counter" || this.subType == "Electric") {
							...
							var WaterMeterDeviceCharacteristic = this.getService(eDomoticzServices.WaterMeterService).getCharacteristic(eDomoticzServices.WaterFlow);
			...
			case this.Type == "P1 Smart Meter" || this.Type == "RFXMeter":
				{
					if ('undefined' !== typeof message.svalue1) {
						if (this.subType == "Gas") {
							...
							var RFXMeterDeviceCharacteristic = this.getService(eDomoticzServices.WaterMeterService).getCharacteristic(eDomoticzServices.WaterFlow);
							var newval8 = Helper.cleanFloat(message.svalue1);
