...
eDomoticzAccessory.prototype = {
    ...
    getColorTempValue: function (service, callback) {
        Domoticz.deviceStatus(this, function (json) {
            ...
            sArray.map(function (s) {
                ...
                const colorTemp = this.getCharacteristic(service, Characteristic.ColorTemperature)

                value = colorTemp.props.minValue + (
                    (colorTemp.props.maxValue - colorTemp.props.minValue) / 255 * tValue
                )
            }.bind(this));
            this.platform.log("Data Received for " + this.name + ": " + value);
            callback(null, value);
    ...
    setColorTempValue: function (value, service, callback, context) {
        if (context && context == "eDomoticz-MQTT") {
            callback();
            return;
        }
        ...
        Domoticz.updateDeviceStatus(this, "setkelvinlevel", {
            "kelvin": (value - colorTemp.props.minValue) / (colorTemp.props.maxValue - colorTemp.props.minValue) * 100
        }, function (success) {
            callback();
        }.bind(this));
    },
            ...
            case this.swTypeVal == Constants.DeviceTypeMedia:
                {
                    ...
                    if (isFan) {
                        ...
                        if (this.subType == "RGBW" || this.subType == "RGBWW" || this.subType == "RGBWWZ") {
                            ...
                            this.gracefullyAddCharacteristic(service, Characteristic.ColorTemperature).on('set', this.setColorTempValue.bind(this, service)).on('get', this.getColorTempValue.bind(this, service));
                        }
                    }
                    this.services.push(service);
                    break;
