...
eDomoticzAccessory.prototype = {
				...
				if (json.result !== undefined) {
					...
					sArray.map(function(s) {
						if (s.SwitchTypeVal == 2) { //contact
							...
							value = Helper.cleanFloat(s.Usage);
		...
		request.get({
			...
			if (!err && response.statusCode == 200) {
				...
				if (json.result !== undefined) {
					var sArray = Helper.sortByKey(json.result, "Name");
					sArray.map(function(s) {
						value = (s.Data == "Open") ? Characteristic.LockCurrentState.UNSECURED : Characteristic.LockCurrentState.UNSECURED;
					}.bind(this));
				}
				this.log("Data Received for " + this.name + ": " + value);
				callback(null, value);
	...
	setLockStatus: function(doorstate,callback,context) {
		...
		command = (doorstate == Characteristic.LockTargetState.UNSECURED)?"On":"Off";
		url = this.control_url + "&switchcmd=" + command;
		request.put({
			url: url,
			header: this.requestHeaders
	...
	handleMQTTMessage: function(message, callback) {
		...
		if ((this.Type == "P1 Smart Meter" && this.swTypeVal == 1 && this.subType == "Gas") || (this.Type == "General" && this.swTypeVal == 2 && this.subType == "Counter Incremental") || (this.name.indexOf("Occupied") > -1) || (this.Type == "General" && this.swTypeVal == 1 && this.subType == "Visibility") || (this.Type == "General" && this.swTypeVal === 0 && this.subType == "kWh") || (this.Type == "General" && this.subType == "Solar Radiation" && this.swTypeVal === 0) || (this.Type == "YouLess Meter" && this.swTypeVal===0) || (this.name.indexOf("Location") > -1)) {
		...
		if (this.isSwitch) {
			...
			case this.swTypeVal == Constants.DeviceTypeSmoke:
				{
					...
					var status = (message.nvalue == 1)?Characteristic.
			...
			case this.swTypeVal == Constants.DeviceDoorLock:
				{
					var doorCurrentChar = this.getService(Service.Lockmechanism).getCharacteristic(Characteristic.LockCurrentState);
					var doorcmd = (parseInt(message.nvalue) == 1) ? Characteristic.LockCurrentState.UNSECURED : Characteristic.LockCurrentState.SECURED;
			...
			case this.Type == "Wind":
				{
					...
					if ('undefined' !== typeof message.svalue2 && 'undefined' !== typeof message.svalue1) {
						...
						var windir = message.svalue2.toString();
						var newval4 = windeg.toString() + ' (' + windir + ')';
						callback(windServiceWindDirectionCharacteristic, newval4);
					} else if ('undefined' !== typeof message.svalue1){
						var windServiceWindDirectionCharacteristic = this.getService(eDomoticzServices.WindDeviceService).getCharacteristic(eDomoticzServices.WindDirection);
						var windDir = Helper.cleanFloat(message.svalue1);
						callback(windServiceWindDirectionCharacteristic, winDir);
					}
					break;
				}
			case this.Type == "Rain":
			...
			case this.swTypeVal == Constants.DeviceDoorLock:
				{
					...
					lockservice.getCharacteristic(Characteristic.LockTargetState).on('set', this.setLockStatus.bind(this));
					lockservice.getCharacteristic(Characteristic.LockCurrentState).on('get', this.getLockStatus.bind(this));
					if (this.batteryRef && this.batteryRef !== 255) { // if batteryRef == 255 we're running on mains
						lockservice.addCharacteristic(new Characteristic.StatusLowBattery()).on('get', this.getLowBatteryStatus.bind(this));
					}
				}
			case this.swTypeVal == Constants.DeviceTypeBlinds:
			case this.swTypeVal == Constants.DeviceTypeBlindsInverted:
